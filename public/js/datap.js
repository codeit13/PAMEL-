$(function () {
	...
	$('#add-the-case').on("click", function () {
		let case_data = [];
	...
	function add_new_insert_files(ds_name, ds_index) {
		...
		$(".file-" + ds_index + "-" + (parseInt(insert_files_count) + 1) +  "").on("change", function() {
	...
	$('#add-new-activity').on("click", function () {
	...
	$('.insert-files-next').on("click", function (event) {
		
		let number_of_repo = 0;

		$('.platform-files-count').each(function (i, obj) {

			let ds_index = $(this).data('index');

			let ds_name = $('[data-index="' + ds_index + '"]').val();

			console.log(ds_index, ds_name);

			add_new_insert_files(ds_name, ds_index);

		});

		$('.step-4-convert-click').on("click", function () {
			event.stopImmediatePropagation();
			event.stopPropagation();
	
			let ds_index = $(this).data('index');
			let ds_name = $(this).data('name');
			console.log(ds_index, ds_name);
	
			var formData = new FormData(document.getElementById('convert-files-' + ds_index));
			...
			$.ajax({
				url: "http://127.0.0.1:8000/aggregateFiles",
				type: 'POST',
	...
	$('.step-5-start').on("click", function () {
		$('.app-name').each(function (i, obj) {
				let ds_name  = $(obj).val();
				...
				$('input[name="Video1' + i + '"]').on('change', function () {
				...
				$('input[name="Video2' + i + '"]').on('change', function () {
				...
				$('input[name="Video3' + i + '"]').on('change', function () {
				...
				$('input[name="Video4' + i + '"]').on('change', function () {
				...
				$('#preparation-add-new-row' + i).on('click', function() {
					add_new_trim_row(i);
				});
				$('#preparation-add-new-column' + i).on('click', function() {
					add_new_trim_column(i);
				});
		});
	});

	$('.step-6-start').on("click", function () {
		$('.app-name').each(function (i, obj) {
				let ds_name  = $(obj).val();
	...
	$('#get-latest-config').on('click', (event) => {
